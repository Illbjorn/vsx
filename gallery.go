...
func (self Gallery) GetExtension(ctx context.Context, publisherID, extensionID, version string) (*bytes.Reader, error) {
	...
	assert(err == nil, "Failed to init GET request: %s.", err)

	// Get the response
	res, err := http.DefaultClient.Do(req)
	assert(err == nil, "Failed to execute GET request to [%s]: %s.", url.String(), err)
	...
	assert(err == nil, "Failed to read extension response body: %s.", err)
